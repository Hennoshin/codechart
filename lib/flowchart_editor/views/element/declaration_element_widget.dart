...
class DeclarationElementWidget extends StatelessWidget {
  final String positionIndex;

  const DeclarationElementWidget({super.key, required this.positionIndex});
    ...
    return GestureDetector(
      ...
      child: Selector2<FlowchartViewModel, FlowchartEditorViewModel, Tuple2<String, bool>>(
          ...
          builder: (_, data, __) => CustomPaint(
            painter: _DeclarationElementPainter(color: data.item2 ? Colors.green : Colors.blue),
...
class _DeclarationElementPainter extends CustomPainter {
  final Color color;

  _DeclarationElementPainter({required this.color});
  ...
  @override
  bool shouldRepaint(covariant _DeclarationElementPainter oldDelegate) {
